...
echo "🔧 Raspberry Pi Home Server Installer (v8.1 - ARM64 Fixed)"
echo "-------------------------------------------------------------------"
echo "🖨️  Mimari: aarch64 (ARM64) için optimize edildi."
echo "-------------------------------------------------------------------"
...
# -------------------------
# 2. RESET (TAM TEMİZLİK VE HATALI İMAJ SİLME)
# -------------------------
if $RESET; then
  ...
  if command -v docker >/dev/null 2>&1; then
    ...
    # Hatalı mimari (exec format error) veren imajları temizle
  ...
  echo "🧹 Eski dosyalar ve bozuk imajlar temizlendi."
fi
...
# -------------------------
# 6. DOCKER COMPOSE OLUŞTURMA (ARM64 ZORLAMALI)
# -------------------------
echo "📝 Docker konfigürasyonu hazırlanıyor (ARM64)..."
...
cd "$BASE_DIR"
# Önceki hatalı imajları zorla güncelle
docker compose pull
docker compose up -d
...
# qBittorrent konteynerinin başlamasını bekle
sleep 5
...
# -------------------------
# 10. PI-HOLE ŞİFRE AYARI (GÜVENLİ BEKLEME)
# -------------------------
echo ""
echo "🔐 PI-HOLE ŞİFRE AYARI"
echo "⏳ Pi-hole servisinin ayağa kalkması bekleniyor (Max 60sn)..."

# Döngü ile konteynerin "Running" durumuna geçmesini bekle
MAX_RETRIES=30
COUNT=0
while [ $COUNT -lt $MAX_RETRIES ]; do
    if [ "$(docker inspect -f '{{.State.Running}}' pihole 2>/dev/null)" == "true" ]; then
        echo "✅ Pi-hole aktif! Şifre ayarlanıyor..."
        sleep 5 # İç servislerin oturması için kısa bir ek bekleme
...
if [ $COUNT -eq $MAX_RETRIES ]; then
    echo "❌ HATA: Pi-hole belirtilen sürede açılamadı. Lütfen 'docker logs pihole' komutu ile hatayı kontrol edin."
fi
...
echo ""
echo "🛑 NOT: Eğer Pi-hole arayüzü açılmazsa (Port 53 hatası), sistemi yeniden başlatın:"
echo "    sudo reboot"